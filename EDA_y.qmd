...
law_structure <- law_set |> 
  filter(법종구분코드명 == "법률") |> 
  select(법령명, 법종구분 = 법종구분코드명) |> 
  left_join(law_nw, by = c("법령명" = "lev1_법령명")) |> 
  mutate(법률체계 = case_when(
...
law_structure |> 
  count(법률체계, name = "법률건수") |> 
  mutate(비율 = 법률건수 / sum(법률건수)) |>
  mutate(법률체계 = factor(법률체계, levels = c("법률", "법률+시행령", "법률+시행령+규칙", "기타"))) |>
  arrange(법률체계) |> 
  gt() |> 
  cols_align("center") |> 
  fmt_integer(법률건수) |>
  fmt_percent(비율, decimals = 1) |>
  tab_header(title = md("**법률, 시행령 시행규칙 위계**"),
             subtitle = "법률+시행령+규칙 = 법률이 시행령과 규칙으로 연결") |> 
  gt_theme_538() |> 
  grand_summary_rows(columns = c(법률건수), 
                     fns = list(합계 = ~sum(.)),
                     fmt = ~fmt_integer(.)) |>
  grand_summary_rows(columns = c(비율),
                     fns = list(합계 = ~sum(법률건수) / sum(법률건수)),
                     fmt = ~fmt_percent(., decimals = 1))
```
...
law_three <- law_set |> 
  ...
  count(법령명, 법률체계, name = "법률건수", sort = TRUE) |> 
  pivot_wider(names_from = 법률체계, values_from = 법률건수, values_fill = 0) |>
  mutate(합계 = 법률 + `법률+시행령` +  `법률+시행령+규칙` ) |> 
  arrange(desc(합계)) |> 
  left_join(law_nw |> select(법령명 =  lev1_법령명, everything()), by = "법령명") 


# law_three |>
#   write_rds("data/law_three.rds")

law_three |> 
  select(법령명, 법률, `법률+시행령` = 시행령, `법률+시행령+규칙` = `시행령+규칙`, 합계) |>
  tail()
  slice_max(합계, n = 20) |> 
  gt() |>
  cols_align("center") |>
  fmt_integer(c(`시행령+규칙`, 시행령, 합계)) |> 
  cols_label(법령명 = "법률",
             `시행령+규칙` = "시행령+규칙",
             시행령 = "시행령") |> 
  tab_header(title = md("**시행령과 시행규칙 갯수 상위 20개 법률**")) |> 
  gt_theme_538() 
...
law_eda |> 
  group_by(법령명, 법령구분, 사무_판단) |>
  summarise(사무수 = sum(사무_판단)) |>
  ungroup() |> 
  pivot_wider(names_from = 사무_판단, values_from = 사무수, values_fill = 0) |> 
  left_join(law_three) |> 
  mutate(총사무수 = `0` + `1`)
```
